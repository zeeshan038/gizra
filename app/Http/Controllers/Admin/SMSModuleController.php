...
class SMSModuleController extends Controller
{
    public function sms_index()
    {
        ...
        $data_values=  Setting::where('settings_type','sms_config')->whereIn('key_name', ['sms4free','twilio','nexmo','2factor','msg91','alphanet_sms'])->orderByRaw("FIELD(key_name,'sms4free','twilio','nexmo','2factor','msg91','alphanet_sms')")->get() ?? [];
    ...
    if ($request['status'] == 1) {
        foreach (['sms4free','twilio','nexmo','2factor','msg91','alphanet_sms'] as $gateway) {
            if ($module != $gateway) {

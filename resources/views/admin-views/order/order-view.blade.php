                    ...
                    <div class="card-body px-0">
                            <?php
                                ...
                                if ($order->is_manual_dispatch == 1) {
                                    $product_price = $order->order_amount - $order->delivery_charge;
                                }
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="media align-items-center">
                                                        <div class="avatar avatar-xl mr-3">
                                                            <img class="img-fluid rounded" src="{{ dynamicAsset('public/assets/admin/img/100x100/food-default-image.png') }}" alt="">
                                                        </div>
                                                        <div class="media-body">
                                                            <strong class="text-primary">{{ translate('Manual Dispatch Delivery Request') }}</strong>
                                                            <span class="d-block font-size-sm text-body mt-1">
                                                                {{ translate('Customer Name:') }} <strong>{{ json_decode($order->delivery_address, true)['contact_person_name'] ?? '' }}</strong><br>
                                                                {{ translate('Phone:') }} <strong>{{ json_decode($order->delivery_address, true)['contact_person_number'] ?? '' }}</strong><br>
                                                                {{ translate('Area:') }} <strong>{{ json_decode($order->delivery_address, true)['address_type'] ?? '' }}</strong><br>
                                                                {{ translate('Address:') }} <strong>{{ json_decode($order->delivery_address, true)['address'] ?? '' }}</strong>
                                                            </span>
                                                        </div>
                                                    </div>
                                                </td>
                                ...
                                <div class="col-12 ">
                                    ...
                                    <div class="card mb-2">
                                        <!-- Header -->
                                                        ...
                                                        @if ($order->is_manual_dispatch == 1 && !$order->delivery_man)
                                                            <div class="w-100 mt-4 text-left border-top pt-3">
                                                                <h5 class="card-title mb-2">
                                                                    <span class="card-header-icon"><i class="tio-send"></i></span>
                                                                    <span>{{translate('Notified Nearby Drivers')}}</span>
                                                                </h5>
                                                                <div class="alert alert-soft-info py-2" style="font-size:11px">
                                                                    {{ translate('Auto-assigned: A request was sent to all active delivery men in zone:') }} <strong>{{ $order->restaurant?->zone?->name ?? 'N/A' }}</strong>.
                                                                </div>
                                                                @if(isset($notifiedDrivers) && count($notifiedDrivers) > 0)
                                                                    <div class="list-group list-group-flush" style="max-height: 250px; overflow-y: auto;">
                                                                        @foreach($notifiedDrivers as $dm)
                                                                            <div class="list-group-item d-flex justify-content-between align-items-center px-0 py-2">
                                                                                <div class="d-flex align-items-center">
                                                                                    <img class="avatar avatar-sm avatar-circle mr-2"
                                                                                         src="{{ $dm->image_full_url ?? dynamicAsset('public/assets/admin/img/160x160/img3.png') }}"
                                                                                         alt="" style="width: 32px; height: 32px;">
                                                                                    <div>
                                                                                        <strong class="d-block font-size-sm">{{ $dm->f_name }} {{ $dm->l_name }}</strong>
                                                                                        <small class="text-muted">{{ $dm->phone }}</small>
                                                                                    </div>
                                                                                </div>
                                                                                <div>
                                                                                    <button type="button" class="btn btn-outline-primary btn-xs" 
                                                                                            onclick="addDeliveryMan({{ $order->id }}, {{ $dm->id }})">
                                                                                        {{ translate('Assign') }}
                                                                                    </button>
                                                                                </div>
                                                                            </div>
                                                                ...
                                                                @else
                                                                    <div class="text-center py-2 text-muted" style="font-size:12px">
                                                                        {{ translate('No active drivers online in this zone.') }}
                                                                    </div>
                                                                @endif
                                                            </div>
                                                        @endif
...
@push('script_2')
    ...
    <script>
        ...
        function addDeliveryMan(orderId, dmId) {
            Swal.fire({
                title: '{{ translate("messages.Are you sure?") }}',
                text: '{{ translate("You want to assign this deliveryman?") }}',
                type: 'warning',
                showCancelButton: true,
                cancelButtonColor: 'default',
                ...
                if (result.value) {
                    $.ajax({
                        url: '{{ url("/") }}/admin/order/add-delivery-man/' + orderId + '/' + dmId,
                        method: 'GET',
                        success: function (data) {
                            toastr.success('{{ translate("messages.deliveryman_assigned_successfully") }}', {
                                CloseButton: true,
                                ProgressBar: true
                            });
                            setTimeout(function () {
                        ...
                        error: function (xhr) {
                            toastr.error(xhr.responseJSON?.message || '{{ translate("messages.failed_to_assign_deliveryman") }}', {
                                CloseButton: true,
                                ProgressBar: true
                            });
                        }

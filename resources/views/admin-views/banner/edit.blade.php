...
<script>
    ...
    $('.banner_type_change').on('change', function (){
        ...
        $('#banner-ai-img-btn').on('click', function () {
            const title = $('input[name="title[]"]').first().val().trim();
            if (!title) { toastr.warning('{{ translate('messages.enter_title_first') }}'); return; }
            const btn = $(this);
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    $('.upload-icon').attr('src', res.image);
                ...
                complete: function () { btn.prop('disabled', false).html('✨ {{ translate('AI Generate Image') }}'); }
            });
        });
        ...
        $('#banner_form').on('submit', function (e) {
            ...
            const hasFile = $('#image-input')[0].files.length > 0;
            ...
            if (base64 && !hasFile) {
                ...
                formData.set('image', new File([new Blob([ab], { type: mime })], 'ai-banner.png', { type: mime }), 'ai-banner.png');
            }
            ...
            $.post({
                ...
                success: function (data) {
                    if (data.errors) {
                        ...
                        toastr.success('{{translate('messages.banner_updated_successfully')}}', { CloseButton: true, ProgressBar: true });

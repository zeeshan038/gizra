...
class SMS_module
{
    ...
    {
        $config = self::get_settings('sms4free');
        if (isset($config) && $config['status'] == 1) {
            return self::sms4free($receiver, $otp, $message);
        }
    ...
    public static function sms4free($receiver, $otp, $message = null): string
    {
        $config = self::get_settings('sms4free');
        $response = 'error';
        if (isset($config) && $config['status'] == 1) {
            if ($message === null) {
                $message = str_replace('#OTP#', $otp, $config['otp_template'] ?? 'קוד האימות שלך: #OTP#');
            }
            // Normalize to Israeli local format: +972XXXXXXXXX → 0XXXXXXXXX
            $phone = preg_replace('/^\+?972/', '0', $receiver);
            $phone = preg_replace('/[^0-9]/', '', $phone);
            // Some stored numbers are missing the leading 0 (e.g. "542065608" instead
            // of "0542065608") — a 9-digit local mobile number always starts with 5.
            if (strlen($phone) === 9 && $phone[0] === '5') {
                $phone = '0' . $phone;
            }

            try {
                $res = \Illuminate\Support\Facades\Http::timeout(15)->post('https://api.sms4free.co.il/ApiSMS/v2/SendSMS', [
                    'key'       => $config['key'],
                    'user'      => $config['user'],
                ...
                $decoded = $res->json();
                $response = (isset($decoded['status']) && (int)$decoded['status'] == 1) ? 'success' : 'error';
            } catch (\Exception $e) {
                $response = 'error';
            }
        }
    ...
    public static function two_factor($receiver, $otp ,$message = null): string
    {
        ...
        if (isset($config) && $config['status'] == 1) {
            ...
            curl_setopt_array($curl, array(
                CURLOPT_URL => "https://2factor.in/API/V1/" . $api_key . "/SMS/" . $receiver . "/" . $otp . "/send_otp/".  "",

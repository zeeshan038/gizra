@extends($admin_mode ?? false ? 'layouts.admin.app' : 'layouts.vendor.app')

@section('title', translate('Listing Manager'))
...
.lm-wrap {
    display: flex;
    height: calc(100vh - {{ ($admin_mode ?? false) ? '80px' : '60px' }});
    background: #f5f6fa;
    overflow: hidden;
}
...
<div class="content container-fluid p-0">
    ...
    <div class="alert alert-soft-warning d-flex align-items-center gap-2 mb-0 border-0 rounded-0" style="font-size:12px;padding:6px 16px">
        ...
        <span>{{ translate('Admin mode') }} — {{ translate('editing') }} <strong>{{ $admin_restaurant_name ?? '' }}</strong>. {{ translate('All changes logged.') }}</span>
        <a href="{{ $admin_back_url ?? url()->previous() }}" class="btn btn-xs btn-light ml-auto">← {{ translate('Back') }}</a>
    </div>
    @endif
    <div class="lm-wrap">
        ...
        <div class="lm-sidebar">
            <div class="lm-sidebar-section">{{ translate('Collections') }}</div>
            <ul class="lm-nav" id="lm-collections">
                <li class="lm-nav-item active" data-filter="all" data-label="{{ translate('All Items') }}">
                    {{ translate('All Items') }}
                    <span class="lm-badge">{{ $all_count }}</span>
                </li>
                <li class="lm-nav-item" data-filter="disabled" data-label="{{ translate('All Disabled') }}">
                    {{ translate('All Disabled') }}
                    <span class="lm-badge">{{ $disabled_count }}</span>
                </li>
                <li class="lm-nav-item" data-filter="drafts" data-label="{{ translate('All Drafts') }}">
                    {{ translate('All Drafts') }}
                    <span class="lm-badge">{{ $drafts_count ?? 0 }}</span>
                </li>
            </ul>
            ...
            <div class="lm-sidebar-section d-flex align-items-center justify-content-between">
                ...
                <span class="d-flex gap-1">
                    <button class="btn btn-xs btn--primary lm-cat-panel-toggle" id="lm-cat-add-btn" title="{{ translate('Add Category') }}" style="padding:1px 6px;font-size:11px">
                        <i class="tio-add"></i>
                    </button>
                    <button class="btn btn-xs btn-outline-secondary lm-cat-panel-toggle" id="lm-cat-view-btn" title="{{ translate('View Categories') }}" style="padding:1px 6px;font-size:11px">
            ...
            <ul class="lm-nav" id="lm-categories">
                ...
                <li class="lm-nav-item" data-filter="category" data-category-id="{{ $cat->id }}" data-label="{{ $cat->name }}">
                    {{ $cat->name }}
                    <span class="lm-badge lm-cat-count" data-id="{{ $cat->id }}">—</span>
                </li>
                @endforeach
            </ul>
        </div>
        ...
        <div id="lm-cat-panel" style="display:none;border-top:1px solid #e7eaf3;background:#fff;padding:12px 14px;min-width:260px;max-width:320px">
            <div class="d-flex align-items-center justify-content-between mb-2">
                <strong style="font-size:13px" id="lm-cat-panel-title">{{ translate('Add Category') }}</strong>
            ...
            <div id="lm-cat-form-wrap">
                <div class="form-group mb-2">
                    <label class="lm-form-label">{{ translate('Name') }} <span class="text-danger">*</span></label>
                    <input type="text" class="form-control form-control-sm" id="lm-cat-name" placeholder="{{ translate('Category name') }}">
                </div>
                <div class="form-group mb-2">
                    <label class="lm-form-label">{{ translate('Image') }}</label>
                    <div id="lm-cat-img-box" style="width:80px;height:60px;border:1px dashed #ccc;border-radius:6px;overflow:hidden;cursor:pointer;position:relative">
                        ...
                        <div id="lm-cat-img-placeholder" style="display:flex;align-items:center;justify-content:center;height:100%;color:#aaa;font-size:11px">+ {{ translate('Image') }}</div>
                    </div>
                    <input type="file" id="lm-cat-img-input" accept="image/*" style="display:none">
                    <input type="hidden" id="lm-cat-img-base64">
                </div>
                <div class="d-flex gap-1 mb-2 flex-wrap">
                    <button class="btn btn-xs btn-outline-secondary" id="lm-cat-ai-img-btn" style="font-size:11px">✨ {{ translate('AI Image') }}</button>
                    <button class="btn btn-xs btn--primary" id="lm-cat-save-btn" style="font-size:11px">{{ translate('Save') }}</button>
                    <button class="btn btn-xs btn-light" id="lm-cat-cancel-btn" style="font-size:11px;display:none">{{ translate('Cancel') }}</button>
                </div>
        ...
        <div class="lm-center">
            ...
            <div class="lm-mobile-filter-bar" id="lm-mobile-filter-bar">
                <span class="lm-cat-pill active" data-filter="all" data-label="{{ translate('All') }}">{{ translate('All') }}</span>
                <span class="lm-cat-pill" data-filter="disabled" data-label="{{ translate('Disabled') }}">{{ translate('Disabled') }}</span>
                <span class="lm-cat-pill" data-filter="drafts" data-label="{{ translate('Drafts') }}">{{ translate('Drafts') }}</span>
                @foreach($categories as $cat)
                <span class="lm-cat-pill" data-filter="category" data-category-id="{{ $cat->id }}" data-label="{{ $cat->name }}">{{ $cat->name }}</span>
                @endforeach
                <button class="lm-cat-pill lm-add-cat-pill" id="lm-mobile-cat-manage" style="border:none;cursor:pointer">
                    <i class="tio-settings" style="font-size:11px"></i> {{ translate('Categories') }}
                </button>
            </div>

            <div class="lm-center-header">
                <div>
                    <h4 id="lm-current-title">{{ translate('All Items') }}</h4>
                ...
                <div class="d-flex gap-2 align-items-center">
                    ...
                    <input type="text" class="form-control form-control-sm" id="lm-search" placeholder="{{ translate('Search items…') }}" style="width:160px">
                    ...
                    <div class="dropdown">
                        <button class="btn btn-outline-primary btn-sm dropdown-toggle" type="button" id="importDropdown" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                            <i class="tio-download-to"></i> {{ translate('Import') }}
                        </button>
                        <div class="dropdown-menu dropdown-menu-right" aria-labelledby="importDropdown" style="z-index: 1050;">
                            <a class="dropdown-item" href="javascript:" id="lm-btn-import-csv"><i class="tio-document-text-outlined mr-1"></i> {{ translate('Import by CSV') }}</a>
                            <a class="dropdown-item" href="javascript:" id="lm-btn-import-pdf"><i class="tio-android-image mr-1"></i> {{ translate('Import by PDF/Image (AI)') }}</a>
                        </div>
                    </div>
        ...
        <div class="lm-detail" id="lm-detail">
            <div class="lm-detail-inner">
                <div class="lm-detail-header">
                    ...
                    <span class="lm-detail-title" id="lm-panel-title">{{ translate('Add Item') }}</span>
                ...
                <ul class="lm-tabs nav" id="lm-tabs">
                    <li class="nav-item">
                        <a class="nav-link active" href="#" data-tab="details">{{ translate('Details') }}</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="#" data-tab="variations">{{ translate('Variations') }}</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="#" data-tab="preview">{{ translate('Preview') }}</a>
                    </li>
                </ul>
                ...
                <div class="lm-tab-body" id="lm-tab-details">
                    ...
                    <div style="display:flex;align-items:flex-start;gap:10px;margin-bottom:12px">
                        <div>
                            <div class="lm-img-upload-box" id="lm-img-box" style="margin-bottom:4px">
                                ...
                                <span class="lm-upload-text">{{ translate('Upload') }}</span>
                                <img id="lm-img-preview" class="lm-img-preview" src="" alt="">
                                <div class="lm-img-change-overlay">
                                    <i class="tio-camera mr-1"></i> {{ translate('Change') }}
                                </div>
                            </div>
                            @if($gemini_ai_enabled == '1')
                            <button type="button" class="lm-ai-img-btn" id="lm-ai-img-btn" title="{{ translate('Generate image with AI') }}">
                                ✨ {{ translate('AI Image') }}
                            </button>
                            @endif
                        </div>
                        ...
                        <div style="flex:1;padding-top:6px">
                            <small class="text-muted" style="font-size:10px;line-height:1.3;display:block">
                                <strong>✨ AI</strong> — {{ translate('Click AI Image to auto-generate a food photo, or AI below description to write it.') }}
                            </small>
                        </div>
                        @endif
                    </div>
                    ...
                    <div class="form-group mb-2">
                        <label class="lm-form-label">{{ translate('Name') }} <span class="text-danger">*</span></label>
                        <input type="text" class="form-control form-control-sm" id="lm-name" placeholder="{{ translate('Item name') }}">
                    </div>

                    <div class="form-group mb-2">
                        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px">
                            <label class="lm-form-label mb-0">{{ translate('Description') }}</label>
                            @if($gemini_ai_enabled == '1')
                            <button type="button" class="lm-ai-btn" id="lm-ai-desc-btn" title="{{ translate('Generate description with AI') }}">
                        ...
                        <textarea class="form-control form-control-sm" id="lm-description" rows="3" placeholder="{{ translate('Short description…') }}"></textarea>
                    </div>

                    <div class="form-group mb-2">
                        <label class="lm-form-label">{{ translate('Category') }} <span class="text-danger">*</span></label>
                    ...
                    <div class="form-group mb-2" id="lm-subcat-wrap" style="display:none">
                        <label class="lm-form-label">{{ translate('Sub Category') }}</label>
                        <select class="form-control form-control-sm" id="lm-subcategory">
                            <option value="">— {{ translate('None') }} —</option>
                        </select>
                    </div>

                    <div class="lm-section-divider">{{ translate('Pricing') }}</div>

                    <div class="form-group mb-2" id="lm-price-wrap">
                        <label class="lm-form-label">
                            {{ translate('Price') }} ({{ \App\CentralLogics\Helpers::currency_symbol() }})
                        ...
                        <div class="lm-price-note d-none" id="lm-price-note">
                            <i class="tio-info-outlined"></i> {{ translate('Price set automatically from first variation option') }}
                        </div>
                    </div>

                    <div class="row g-2 mb-2">
                        <div class="col-6">
                            <label class="lm-form-label">{{ translate('Discount') }}</label>
                            <input type="number" class="form-control form-control-sm" id="lm-discount" min="0" step="0.01" value="0">
                        </div>
                        <div class="col-6">
                            <label class="lm-form-label">{{ translate('Discount Type') }}</label>
                    ...
                    <div class="form-group mb-2">
                        <label class="lm-form-label">{{ translate('Stock Type') }}</label>
                        <select class="form-control form-control-sm" id="lm-stock-type">
                            <option value="unlimited">{{ translate('Unlimited Stock') }}</option>
                            <option value="limited">{{ translate('Limited Stock') }}</option>
                            <option value="daily">{{ translate('Daily Stock') }}</option>
                        </select>
                    </div>
                    <div class="form-group mb-2 d-none" id="lm-item-stock-wrap">
                        <label class="lm-form-label">{{ translate('Item Stock') }} <span class="text-danger">*</span></label>
                        <input type="number" class="form-control form-control-sm" id="lm-item-stock" min="0" max="999999999" placeholder="{{ translate('Ex: 100') }}">
                    </div>

                    <div class="lm-section-divider">{{ translate('Availability Time') }}</div>
                    <div class="row g-2 mb-2">
                        <div class="col-6">
                            <label class="lm-form-label">{{ translate('Available From') }} <span class="text-danger">*</span></label>
                            <input type="time" class="form-control form-control-sm" id="lm-time-start" value="00:00">
                        </div>
                        <div class="col-6">
                            <label class="lm-form-label">{{ translate('Available Until') }} <span class="text-danger">*</span></label>
                    ...
                    <div class="lm-status-row">
                        <span class="lm-form-label mb-0">{{ translate('Type') }}</span>
                        <div class="d-flex gap-2">
                            <label class="mb-0 d-flex align-items-center" style="cursor:pointer">
                                ...
                                <span class="lm-veg-dot veg"></span> {{ translate('Veg') }}
                            </label>
                            <label class="mb-0 d-flex align-items-center" style="cursor:pointer">
                                <input type="radio" name="lm-veg" id="lm-veg-0" value="0" class="mr-1" checked>
                                <span class="lm-veg-dot nonveg"></span> {{ translate('Non-Veg') }}
                            </label>
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="lm-status-row">
                        <span class="lm-form-label mb-0">{{ translate('Status') }}</span>
                ...
                <div class="lm-tab-body d-none" id="lm-tab-variations">
                    <p class="text-muted" style="font-size:12px">
                        {{ translate('Create variation groups (e.g. Size, Toppings). When variations exist, the base price is set from the first option automatically.') }}
                    </p>
                    <div id="lm-var-groups"></div>
                    <button class="lm-add-group-btn" id="lm-add-group-btn">
                        <i class="tio-add"></i> {{ translate('Add Variation Group') }}
                    </button>
                </div>
                ...
                <div class="lm-tab-body d-none" id="lm-tab-preview">
                    <p class="text-muted mb-3" style="font-size:11px">
                        <i class="tio-info-outlined"></i> {{ translate('This is how customers will see it in the app.') }}
                    </p>
                    ...
                    <div class="lm-preview-live-badge">
                        <i class="tio-info-outlined"></i> {{ translate('Live preview updates automatically') }}
                    </div>
                </div>

                <div class="lm-detail-footer d-flex gap-2">
                    <button class="btn btn--primary flex-fill" id="lm-save-btn">{{ translate('Save') }}</button>
                    <button class="btn btn-success flex-fill d-none" id="lm-approve-btn">{{ translate('Approve & Publish') }}</button>
                </div>
            </div>
        </div>
...
(function() {
    ...
    const BASE   = '{{ $lm_base ?? route("vendor.listing-manager.index") }}';
    const globalCategories = @json($categories);
    const CUR_SYM = '{{ \App\CentralLogics\Helpers::currency_symbol() }}';

    let currentFilter   = 'all';
    let currentCatId    = null;
    let currentLabel    = '{{ translate("All Items") }}';
    ...
    function updateDraftCategoryDropdown(items) {
        ...
        select.empty();
        select.append(`<option value="">{{ translate('All Categories') }} (${items.length})</option>`);
    ...
    function loadItems() {
        ...
        $.get(BASE + '/items', params, function(res) {
            ...
            $('#lm-items-body').html('<div class="lm-empty"><i class="tio-warning-outined"></i>{{ translate("Failed to load items") }}</div>');
        });
    }
    ...
    function renderItems(items) {
        ...
        if (!filtered.length) {
            $('#lm-items-body').html('<div class="lm-empty"><i class="tio-inbox-outlined"></i>{{ translate("No items found") }}</div>');
            return;
        }
        let html = '';
        filtered.forEach(item => {
            const price = item.has_variations
                ? CUR_SYM + parseFloat(item.display_price).toFixed(2) + '<small style="font-weight:400"> {{ translate("from") }}</small>'
                : CUR_SYM + parseFloat(item.display_price).toFixed(2);
            const vegDot = @json(config('toggle_veg_non_veg')) ? `<span class="lm-veg-dot ${item.veg ? 'veg' : 'nonveg'}"></span>` : '';
            const statusColor = item.status ? '#28a745' : '#dc3545';
            const draftBadge = item.is_draft ? `<span class="badge badge-warning ml-1" style="font-size:10px;vertical-align:middle">{{ translate("Draft") }}</span>` : '';
            ...
            const publishBtn = item.is_draft
                ? `<button class="btn btn-sm btn-outline-success lm-publish-btn mr-2" data-id="${item.id}" title="{{ translate('Publish') }}"><i class="tio-publish"></i></button>`
                : '';

            html += `
            <div class="lm-item-row ${currentFoodId == item.id ? 'active' : ''}" data-id="${item.id}">
                ${selectCheckbox}
                <img class="lm-item-img" src="${item.image_full_url || '{{ dynamicAsset("public/assets/admin/img/100x100/no-image-found.png") }}'}" alt="" onerror="this.src='{{ dynamicAsset("public/assets/admin/img/100x100/no-image-found.png") }}'">
                <div class="lm-item-info">
                    <div class="lm-item-name">${item.name}${draftBadge}</div>
                    <div class="lm-item-sub">${vegDot}${item.has_variations ? '{{ translate("Variable item") }}' : '{{ translate("Simple item") }}'}</div>
                </div>
                <div class="lm-item-price">${price}</div>
                <div class="lm-item-actions">
                    ${publishBtn}
                    <div class="custom-control custom-switch" title="${item.status ? '{{ translate("Active") }}' : '{{ translate("Inactive") }}'}">
                    ...
                    <button class="btn btn-sm btn-outline-danger lm-del-btn" data-id="${item.id}" title="{{ translate('Delete') }}">
    ...
    function openItem(id) {
        ...
        openPanel('{{ translate("Edit Item") }}');
        resetForm();
        switchTab('details');
        $('#lm-save-btn').prop('disabled', true);
        $('#lm-panel-title').html('{{ translate("Edit Item") }}&nbsp;<i class="tio-sync tio-anim-spin" id="lm-panel-loader" style="font-size:13px;color:#aaa"></i>');

        $.get(BASE + '/item/' + id, function(item) {
            $('#lm-panel-loader').remove();
            $('#lm-panel-title').text('{{ translate("Edit Item") }}');
            ...
            $('#lm-panel-title').text('{{ translate("Edit Item") }}');
            $('#lm-save-btn').prop('disabled', false);
            showToast('{{ translate("Failed to load item") }}', 'danger');
        });
    }
    ...
    function renderPreview() {
        ...
        groups.each(function() {
            ...
            const subtitle = isSingle ? '{{ translate("Please select one") }}' : '{{ translate("Choose as many as you like") }}';
            ...
            <div class="lm-preview-var-group">
                <div class="lm-preview-var-title">
                    ${escHtml(gName)}
                    ${req ? '<span class="lm-preview-var-required">&#10033; {{ translate("Required") }}</span>' : ''}
                </div>
    ...
    function resetForm() {
        ...
        $('#lm-description').val('');
        $('#lm-category').val('');
        $('#lm-subcategory').html('<option value="">— {{ translate("None") }} —</option>');
        $('#lm-subcat-wrap').hide();
    ...
    function loadSubcategories(catId, selectedId) {
        ...
        $.get(BASE + '/get-categories', { parent_id: catId }, function(data) {
            ...
            let opts = '<option value="">— {{ translate("None") }} —</option>';
            data.forEach(c => {
    ...
    function addVariationGroup(data) {
        ...
        <div class="lm-var-group" data-group="${gIdx}">
            ...
            <div class="lm-var-group-header">
                ...
                <span class="lm-var-group-name">${name || '{{ translate("New Group") }}'}</span>
                <button type="button" class="btn btn-sm btn-link text-danger p-0 lm-del-group" title="{{ translate('Delete Group') }}">
                    <i class="tio-delete-outlined"></i>
                </button>
            </div>
            <div class="lm-var-group-body">
                <div class="form-group mb-2">
                    <label class="lm-form-label">{{ translate('Group Name') }}</label>
                    <input type="text" class="form-control form-control-sm lm-group-name-input" value="${escHtml(name)}" placeholder="{{ translate('e.g. Size, Toppings') }}">
                </div>
                <div class="lm-type-btn-group">
                    <button type="button" class="lm-type-btn ${type === 'single' ? 'active' : ''}" data-type="single">
                        <i class="tio-checkmark-circle-outlined"></i> {{ translate('Single Choice') }}
                    </button>
                    <button type="button" class="lm-type-btn ${type === 'multi' ? 'active' : ''}" data-type="multi">
                        <i class="tio-checkmark-square-outlined"></i> {{ translate('Multiple Choice') }}
                    </button>
                </div>
    ...
    function buildOptionRow(gIdx, oIdx, label, price, optId) {
        ...
        <div class="lm-option-row" data-opt="${oIdx}">
            ...
            <input type="text" class="form-control form-control-sm lm-opt-label" placeholder="{{ translate('Option name') }}" value="${escHtml(label)}">
            <input type="number" class="form-control form-control-sm lm-opt-price" placeholder="{{ translate('Price') }}" min="0" step="0.01" value="${price}">
            <button type="button" class="btn-del-opt" title="{{ translate('Remove') }}">&times;</button>
        </div>`;
    }

    // Group name syncs to header
    $(document).on('input', '.lm-group-name-input', function() {
        $(this).closest('.lm-var-group').find('.lm-var-group-name').text($(this).val() || '{{ translate("New Group") }}');
    });
    ...
    function saveItem() {
        ...
        if (!name) { alert('{{ translate("Item name is required") }}'); switchTab('details'); $('#lm-name').focus(); return; }
        if (!catId) { alert('{{ translate("Category is required") }}'); switchTab('details'); $('#lm-category').focus(); return; }
        if (!timeStart || !timeEnd) { alert('{{ translate("Availability time is required") }}'); switchTab('details'); return; }
        if (timeStart >= timeEnd) { alert('{{ translate("Available Until must be after Available From") }}'); switchTab('details'); $('#lm-time-end').focus(); return; }
        if (!groups.length && (!price || parseFloat(price) <= 0)) {
            alert('{{ translate("Price is required when no variations are set") }}'); switchTab('details'); $('#lm-price').focus(); return;
        }
        ...
        groups.each(function() {
            ...
            if (!gName) { alert('{{ translate("Variation group name is required") }}'); switchTab('variations'); valid = false; return false; }
            if (!opts.length) { alert('{{ translate("Each variation group must have at least one option") }}'); switchTab('variations'); valid = false; return false; }
        });
        if (!valid) return;
        ...
        if (stockType !== 'unlimited') {
            ...
            if (!itemStock || parseInt(itemStock) < 0) {
                alert('{{ translate("Item stock is required for limited/daily stock type") }}');
                switchTab('details');
                $('#lm-item-stock').focus();
                return;
            }
        ...
        $('#lm-save-btn').prop('disabled', true).text('{{ translate("Saving…") }}');
        ...
        $.ajax({
            ...
            success: function(res) {
                if (res.errors && res.errors.length) {
                    ...
                    showToast('{{ translate("Saved successfully!") }}', 'success');
                }
            },
            error: function(xhr) {
                let msg = '{{ translate("Failed to save") }}';
                try { msg = xhr.responseJSON.errors[0].message || msg; } catch(e) {}
                alert(msg);
            },
            complete: function() {
                $('#lm-save-btn').prop('disabled', false).text('{{ translate("Save") }}');
            }
        });
    }
    ...
    $(document).on('click', '.lm-del-btn', function(e) {
        ...
        $.ajax({
            ...
            success: function() {
                ...
                renderItems(allItems);
                refreshAllCounts();
                showToast('{{ translate("Item deleted") }}', 'danger');
            },
            error: function() { alert('{{ translate("Delete failed") }}'); }
        });
    });
    ...
    $(document).on('change', '.lm-status-toggle', function(e) {
        ...
        $.post(BASE + '/toggle-status', { _token: CSRF, id: id, status: val }, function(res) {
            ...
            alert('{{ translate("Status update failed") }}');
        });
    });
    ...
    function getAiItemName() {
        ...
        if (!n) { alert('{{ translate("Enter item name first to generate AI content") }}'); $('#lm-name').focus(); return null; }
        return n;
    }
    ...
    $('#lm-ai-desc-btn').on('click', function() {
        ...
        $.ajax({
            ...
            error: function(xhr) {
                let msg = '{{ translate("AI generation failed") }}';
    ...
    $('#lm-ai-img-btn').on('click', function() {
        ...
        if (!name) return;
        const btn = $(this);
        btn.prop('disabled', true).html('<i class="tio-sync tio-anim-spin" style="font-size:10px"></i> {{ translate("Gen…") }}');
        $.ajax({
            url: BASE + '/ai-image',
            method: 'POST',
            headers: { 'X-CSRF-TOKEN': CSRF },
            ...
            error: function(xhr) {
                let msg = '{{ translate("AI image generation failed") }}';
                try { msg = xhr.responseJSON.error || msg; } catch(e) {}
                alert(msg);
            },
            complete: function() {
                btn.prop('disabled', false).html('✨ {{ translate("AI Image") }}');
            }
        });
    });
    ...
    function resetCatForm() {
        ...
        catImageFile = null;
        catImageBase64 = null;
        $('#lm-cat-panel-title').text('{{ translate("Add Category") }}');
        $('#lm-cat-cancel-btn').hide();
        $('#lm-cat-save-btn').text('{{ translate("Save") }}');
    }

    function loadCatList() {
        $.get(CAT_BASE, function(cats) {
            if (!cats.length) {
                $('#lm-cat-list').html('<p class="text-muted" style="font-size:11px;margin:4px 0">{{ translate("No categories yet") }}</p>');
                return;
            }
            let html = '';
            cats.forEach(function(c) {
                ...
                const badge = c.status == 1 ? '' : ' <span style="font-size:9px;color:#dc3545">({{ translate("Disabled") }})</span>';
    ...
    function refreshSidebarCategories() {
        $.get(CAT_BASE, function(cats) {
            ...
            let optHtml = '<option value="">— {{ translate("Select category") }} —</option>';
            cats.forEach(function(c) {
    ...
    $('#lm-cat-ai-img-btn').on('click', function() {
        ...
        if (!name) { alert('{{ translate("Enter category name first") }}'); return; }
        const btn = $(this);
        ...
        $.ajax({
            ...
            error: function(xhr) {
                let msg = '{{ translate("AI image failed") }}';
                try { msg = xhr.responseJSON.error || msg; } catch(e) {}
                alert(msg);
            },
            complete: function() { btn.prop('disabled', false).html('✨ {{ translate("AI Image") }}'); }
        });
    });
    ...
    $('#lm-cat-save-btn').on('click', function() {
        const name = $('#lm-cat-name').val().trim();
        if (!name) { alert('{{ translate("Name is required") }}'); return; }
    ...
    $(document).on('click', '.lm-cat-edit', function() {
        ...
        $.get(CAT_BASE, function(cats) {
            ...
            $('#lm-cat-panel-title').text('{{ translate("Edit Category") }}');
            $('#lm-cat-cancel-btn').show();
            $('#lm-cat-save-btn').text('{{ translate("Update") }}');
            $('#lm-cat-name').focus();
        });
    });
    ...
    $(document).on('click', '.lm-cat-delete', function() {
        const id = $(this).data('id');
        if (!confirm('{{ translate("Delete this category?") }}')) return;
        $.ajax({
            url: CAT_BASE + '/' + id,
            method: 'DELETE',
            headers: { 'X-CSRF-TOKEN': CSRF },
    ...
    function runImportProgress(modalId, formId, parseUrl, formData) {
        ...
        progressTitle.text(parseUrl.includes('pdf') ? '{{ translate("Gemini AI Scanning PDF Menu...") }}' : '{{ translate("Reading CSV file...") }}');
        progressStatus.text('{{ translate("Parsing items and categories...") }}');
        ...
        $.ajax({
            ...
            success: function(res) {
                const items = res.items || [];
                if (!items.length) {
                    showToast('{{ translate("No items parsed from file.") }}', 'warning');
                    resetModal();
                    errorAlert.removeClass('d-none').text('{{ translate("No items parsed from file. Please verify content format.") }}');
                    return;
                }

                progressTitle.text('{{ translate("Adding items to drafts...") }}');
                let importedCount = 0;
    ...
    function updateSelectedCount() {
        ...
        $('#lm-selected-count').text(count + ' ' + '{{ translate("items selected") }}');
    }
    ...
    $(document).on('click', '.lm-publish-btn', function(e) {
        ...
        $.post(BASE + '/bulk-publish', { _token: CSRF, ids: [id] }, function(res) {
            ...
            showToast('{{ translate("Publish failed") }}', 'danger');
    ...
    $(document).on('click', '#lm-approve-btn', function() {
        ...
        if (!id) return;
        const btn = $(this);
        btn.prop('disabled', true).text('{{ translate("Approving…") }}');
        ...
        $.post(BASE + '/approve/' + id, { _token: CSRF }, function(res) {
            ...
            showToast('{{ translate("Approve failed") }}', 'danger');
        }).always(function() {
            btn.prop('disabled', false).text('{{ translate("Approve & Publish") }}');
        });
    });
})();
</script>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">{{ translate('Import Menu via CSV') }}</h5>
            ...
            <form id="lm-csv-form" enctype="multipart/form-data">
                <div class="modal-body">
                    ...
                    <div class="lm-form-fields">
                        ...
                        <div class="alert alert-info shadow-sm" style="font-size:12px; background-color:#343a40; color:#fff; border-color:#343a40;">
                            <strong>{{ translate('CSV Format:') }}</strong><br>
                            ...
                            <a href="data:text/csv;charset=utf-8,Category,Name,Description,Price,Item%20Type,Veg,Variations%0AStarters,Spring%20Rolls,Crispy%20veggie%20rolls,5.99,Simple,Veg,%0AMains,Cheese%20Pizza,Delicious%20mozzarella%20pizza,12.99,Variable,Veg,Size:Small%3D0%2CMedium%3D2.5%2CLarge%3D5.0%0ADrinks,Lemonade,Fresh%20squeezed%20lemons,3.50,Simple,Veg," download="menu_template.csv" class="text-white font-weight-bold" style="text-decoration: underline;">
                                <i class="tio-download-to"></i> {{ translate('Download Template') }}
                            </a>
                        </div>
                    </div>
                    ...
                    <div class="text-center d-none lm-progress-container">
                        ...
                        <h4 class="mt-3 text-primary lm-progress-title">{{ translate('Reading CSV...') }}</h4>
                ...
                <div class="modal-footer lm-modal-footer">
                    ...
                    <button type="submit" class="btn btn--primary" id="lm-csv-submit">{{ translate('Import') }}</button>
                </div>
            </form>
        </div>
    </div>
        ...
        <div class="modal-content">
            ...
            <form id="lm-pdf-form" enctype="multipart/form-data">
                <div class="modal-body">
                    ...
                    <div class="lm-form-fields">
                        <div class="form-group">
                            <label>{{ translate('Upload Menu PDF or Image') }}</label>
                            <input type="file" name="menu_file" class="form-control-file" accept=".pdf,.jpg,.jpeg,.png" required>
                        </div>
                        <div class="alert alert-info" style="font-size:12px">
                            {{ translate('Upload a PDF or clear image of your menu. Gemini AI will scan it, extract categories, items, prices, and variations, and load them into drafts.') }}
                        </div>
                    </div>
                    ...
                    <div class="text-center d-none lm-progress-container">
                        ...
                        <h4 class="mt-3 text-primary lm-progress-title">{{ translate('Scanning PDF Menu...') }}</h4>
                        <div class="progress mt-3" style="height: 18px; border-radius: 9px;">
                            <div class="progress-bar progress-bar-striped progress-bar-animated bg-success lm-progress-bar" role="progressbar" style="width: 0%; border-radius: 9px;">0%</div>
                        </div>
                        <p class="mt-2 text-muted lm-progress-status" style="font-size:12px;"></p>
                ...
                <div class="modal-footer lm-modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">{{ translate('Cancel') }}</button>
                    <button type="submit" class="btn btn--primary" id="lm-pdf-submit">{{ translate('Analyze & Import') }}</button>
                </div>
            </form>
        </div>
    </div>

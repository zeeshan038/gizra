...
Route::group(['namespace' => 'Api\V1', 'middleware'=>['localization','react']], function () {
    ...
    Route::group(['prefix' => 'vendor', 'namespace' => 'Vendor', 'middleware'=>['vendor.api']], function () {
        ...
        Route::get('manual-orders-settlement', 'VendorController@getManualOrdersSettlement');
        ...
        // Manual driver dispatch for phone orders. Additive: no existing
        // route, controller or flow is modified by this group.
        Route::group(['prefix'=>'dispatch'], function(){
            Route::post('request-driver', 'DispatchController@request_driver');
        });
        ...
        // Listing Manager
        Route::group(['prefix' => 'listing-manager'], function () {
            // Food items
            Route::get('items', 'ListingManagerController@getItems');
            Route::get('item/{id}', 'ListingManagerController@getItem');
            Route::post('store', 'ListingManagerController@store');
            Route::post('update/{id}', 'ListingManagerController@update');
            Route::delete('delete/{id}', 'ListingManagerController@delete');
            Route::post('toggle-status', 'ListingManagerController@toggleStatus');
            Route::get('get-categories', 'ListingManagerController@getCategories');

            // AI Helpers
            Route::post('ai-description', 'ListingManagerController@aiDescription');
            Route::post('ai-image', 'ListingManagerController@aiImage');

            // Category CRUD
            Route::get('categories', 'ListingManagerController@categoryList');
            Route::post('categories', 'ListingManagerController@categoryStore');
            Route::post('categories/ai-image', 'ListingManagerController@categoryAiImage');
            Route::post('categories/update/{id}', 'ListingManagerController@categoryUpdate');
            Route::delete('categories/delete/{id}', 'ListingManagerController@categoryDelete');

            // Addon CRUD
            Route::get('addons', 'ListingManagerController@addonList');
            Route::post('addons', 'ListingManagerController@addonStore');
            Route::post('addons/update/{id}', 'ListingManagerController@addonUpdate');
            Route::delete('addons/delete/{id}', 'ListingManagerController@addonDelete');
            Route::post('addons/toggle-status', 'ListingManagerController@addonToggleStatus');
        });
    ...
    // Partner integration API — reuses the existing VendorController methods
    // unmodified. Auth/scoping happens entirely in the partner.* middleware,
    // which resolves X-Gizra-Key -> restaurant -> vendor and sets
    // $request['vendor'] the same way VendorTokenIsValid already does for the
    // in-app vendor session, so these controllers behave identically either way.
    Route::group(['prefix' => 'partner', 'namespace' => 'Vendor'], function () {
        Route::middleware(['partner.auth:orders:read', 'partner.signature', 'partner.throttle:read'])->group(function () {
            Route::get('profile', 'VendorController@get_profile');
            Route::get('current-orders', 'VendorController@get_current_orders');
            Route::get('completed-orders', 'VendorController@get_completed_orders');
            Route::get('order', 'VendorController@get_order');
        ...
        Route::middleware(['partner.auth:orders:write', 'partner.signature', 'partner.throttle:write'])->group(function () {
            Route::put('update-order-status', 'VendorController@update_order_status');
        });
    });
    ...
    Route::group(['prefix' => 'customer', 'middleware' => 'apiGuestCheck'], function () {
        Route::group(['prefix' => 'order'], function () {
            ...
            Route::post('generate-payment-link', 'OrderController@generate_payment_link');

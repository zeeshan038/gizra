...
trait  SmsGateway
{
    public static function send($receiver, $otp): string
    {
        ...
        if (isset($config) && $config['status'] == 1) {
            return self::sms4free($receiver, $otp);
        }
    ...
    public static function sms4free($receiver, $otp, $message = null): string
    {
        ...
        if (isset($config) && $config['status'] == 1) {
            ...
            try {
                $res = \Illuminate\Support\Facades\Http::timeout(15)->post('https://www.sms4free.co.il/ApiSMS/v2/SendSMS', [
                    'key'       => $config['key'],
                    'user'      => $config['user'],
                ...
                $body = $res->body();
                $response = (is_numeric($body) && (int)$body > 0) ? 'success' : 'error';
            } catch (\Exception $e) {
                $response = 'error';
            }
        }

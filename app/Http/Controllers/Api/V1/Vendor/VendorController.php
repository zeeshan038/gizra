...
class VendorController extends Controller
{
    ...
    public function get_completed_orders(Request $request)
    {
        ...
        ->when($request->status == 'all', function($query){
            return $query->where(function($q){
                $q->whereIn('order_status', ['refunded','refund_requested','refund_request_canceled', 'delivered','canceled','failed' ])
    ...
    public function getManualOrdersSettlement(Request $request)
    {
        $vendor = $request['vendor'];
        $restaurant = $vendor->restaurants[0];
        ...
        foreach ($orders as $order) {
            ...
            $commPercent = 0.10;
            $commAmt = ($order->order_amount * $commPercent) + (float)$order->original_delivery_charge;

            if ($prepaid) {
                $totalPayable += $commAmt;
            } else {
                $totalReceivable += ($order->order_amount - $commAmt);
            }
        }
        ...
        return response()->json([
            'total_manual_orders' => $orders->count(),
            'total_payable_to_gizra' => round($totalPayable, 2),
            'total_receivable_from_gizra' => round($totalReceivable, 2),
            'net_balance' => round(abs($netBalance), 2),
            'net_balance_type' => $netBalance >= 0 ? 'payable' : 'receivable'
        ]);
    }

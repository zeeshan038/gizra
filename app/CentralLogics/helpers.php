    ...
    public static function order_data_formatting($data, $multi_data = false)
    {
        ...
        if ($multi_data) {
            foreach ($data as $item) {
                ...
                unset($item['details']);
                if ($item['is_manual_dispatch'] == 1) {
                    $prepaid = ($item['payment_status'] === 'paid');
                    $commPercent = 0.10;
                    $commAmt = ($item['order_amount'] * $commPercent) + (float)$item['original_delivery_charge'];
                    if ($prepaid) {
                        $item['manual_order_settlement_type'] = 'payable';
                        $item['manual_order_settlement_amount'] = (float)round($commAmt, 2);
                    } else {
                        $item['manual_order_settlement_type'] = 'receivable';
                        $item['manual_order_settlement_amount'] = (float)round($item['order_amount'] - $commAmt, 2);
                    }
                } else {
                    $item['manual_order_settlement_type'] = null;
                    $item['manual_order_settlement_amount'] = 0.00;
                }
                array_push($storage, $item);
            }
            $data = $storage;
            ...
            unset($data['details']);
            if ($data['is_manual_dispatch'] == 1) {
                $prepaid = ($data['payment_status'] === 'paid');
                $commPercent = 0.10;
                $commAmt = ($data['order_amount'] * $commPercent) + (float)$data['original_delivery_charge'];
                if ($prepaid) {
                    $data['manual_order_settlement_type'] = 'payable';
                    $data['manual_order_settlement_amount'] = (float)round($commAmt, 2);
                } else {
                    $data['manual_order_settlement_type'] = 'receivable';
                    $data['manual_order_settlement_amount'] = (float)round($data['order_amount'] - $commAmt, 2);
                }
            } else {
                $data['manual_order_settlement_type'] = null;
                $data['manual_order_settlement_amount'] = 0.00;
            }
        }
        return $data;
    }
...
//            $distance = data_get($distance,'rows.0.elements.0.distance.text',' ');
    ...
    {
        $order= Order::where('id',$order->id)->with('zone:id,deliveryman_wise_topic','restaurant:id,name,restaurant_model,self_delivery_system,vendor_id','restaurant.restaurant_sub','customer:id,cm_firebase_token,email,f_name,l_name,current_language_key,phone','restaurant.vendor:id,firebase_token,phone','delivery_man:id,fcm_token,phone','guest')->first();
        ...
        try {
            ...
            // ── SMS Notifications via SMS4Free ────────────────────────────────
            try {
                // Customer SMS — order status updates
                $cust_sms_status = Helpers::getNotificationStatusData('customer', 'customer_order_notification');
                if ($cust_sms_status?->sms_status == 'active' && !$order->is_guest) {
                    $customer = $order->customer;
                    if ($customer && $customer->phone) {
                        $smsText = self::order_status_update_message($status, 'he');
                        if ($smsText && $smsText !== '0') {
                            $smsText = self::text_variable_data_format(
                                value: $smsText,
                                user_name: "{$customer->f_name} {$customer->l_name}",
                                restaurant_name: $order->restaurant?->name,
                                order_id: $order->id
                            );
                            $res = SMS_module::send($customer->phone, null, $smsText);
                            \Illuminate\Support\Facades\Log::channel('daily')->info('[SMS ORDER] customer', ['order_id' => $order->id, 'phone' => $customer->phone, 'result' => $res]);
                        }
                    }
                } else {
                    \Illuminate\Support\Facades\Log::channel('daily')->info('[SMS ORDER] customer skipped', ['order_id' => $order->id, 'sms_status' => $cust_sms_status?->sms_status, 'is_guest' => $order->is_guest]);
                }

                // Restaurant SMS — new order placed
                $rest_sms_status = Helpers::getNotificationStatusData('restaurant', 'restaurant_order_notification');
                if ($rest_sms_status?->sms_status == 'active') {
                    $isNewOrder = (
                        ($order->order_status == 'pending' && in_array($order->payment_method, ['cash_on_delivery', 'offline_payment']))
                        || ($order->order_status == 'confirmed' && !in_array($order->payment_method, ['cash_on_delivery', 'offline_payment']))
                    );
                    if ($isNewOrder && $order->restaurant?->vendor) {
                        $vendorPhone = $order->restaurant->vendor->phone ?? null;
                        if ($vendorPhone) {
                            $restMsg = 'הזמנה חדשה #' . $order->id . ' התקבלה במסעדה שלך. סכום: ₪' . number_format($order->order_amount, 2) . '. אנא אשר בהקדם.';
                            $res = SMS_module::send($vendorPhone, null, $restMsg);
                            \Illuminate\Support\Facades\Log::channel('daily')->info('[SMS ORDER] restaurant', ['order_id' => $order->id, 'restaurant_id' => $order->restaurant_id, 'phone' => $vendorPhone, 'result' => $res]);
                        } else {
                            \Illuminate\Support\Facades\Log::channel('daily')->warning('[SMS ORDER] restaurant skipped — no vendor phone', ['order_id' => $order->id, 'restaurant_id' => $order->restaurant_id]);
                        }
                    } else {
                        \Illuminate\Support\Facades\Log::channel('daily')->info('[SMS ORDER] restaurant skipped', ['order_id' => $order->id, 'order_status' => $order->order_status, 'payment_method' => $order->payment_method, 'isNewOrder' => $isNewOrder, 'has_vendor' => (bool) $order->restaurant?->vendor]);
                    }
                } else {
                    \Illuminate\Support\Facades\Log::channel('daily')->info('[SMS ORDER] restaurant notification inactive', ['order_id' => $order->id, 'sms_status' => $rest_sms_status?->sms_status]);
                }

                // Deliveryman SMS — broadcast to ALL active drivers once restaurant confirms
                // the order, so any of them can pick it up. Stops the moment a driver
                // accepts (order_status becomes 'accepted') — a driver_broadcast_sent_at
                // flag prevents re-broadcasting on every subsequent status-check call.
                $dm_sms_status = Helpers::getNotificationStatusData('deliveryman', 'deliveryman_order_notification');
                if ($dm_sms_status?->sms_status == 'active') {
                    if ($order->order_status == 'confirmed' && !$order->delivery_man_id && !$order->driver_broadcast_sent_at) {
                        $drivers = DeliveryMan::active()->whereNotNull('phone')->where('phone', '!=', '')->get();
                        $address = json_decode($order->delivery_address, true)['address'] ?? '';
                        $successCount = 0;
                        foreach ($drivers as $driver) {
                            $dmMsg = 'הזמנה חדשה זמינה לאיסוף #' . $order->id . '. כתובת: ' . $address . '. פתח את האפליקציה כדי לקחת אותה.';
                            $res = SMS_module::send($driver->phone, null, $dmMsg);
                            \Illuminate\Support\Facades\Log::channel('daily')->info('[SMS ORDER] deliveryman broadcast', ['order_id' => $order->id, 'delivery_man_id' => $driver->id, 'phone' => $driver->phone, 'result' => $res]);
                            if ($res === 'success') {
                                $successCount++;
                            }
                        }
                        $order->driver_broadcast_sent_at = now();
                        $order->save();
                        \Illuminate\Support\Facades\Log::channel('daily')->info('[SMS ORDER] deliveryman broadcast summary', ['order_id' => $order->id, 'drivers_notified' => $drivers->count(), 'success_count' => $successCount]);
                    } elseif ($order->order_status == 'accepted') {
                        \Illuminate\Support\Facades\Log::channel('daily')->info('[SMS ORDER] deliveryman broadcast stopped — order already accepted', ['order_id' => $order->id, 'delivery_man_id' => $order->delivery_man_id]);
                    } else {
                        \Illuminate\Support\Facades\Log::channel('daily')->info('[SMS ORDER] deliveryman broadcast skipped', ['order_id' => $order->id, 'order_status' => $order->order_status, 'has_delivery_man' => (bool) $order->delivery_man_id, 'already_broadcast' => (bool) $order->driver_broadcast_sent_at]);
                    }
                } else {
                    \Illuminate\Support\Facades\Log::channel('daily')->info('[SMS ORDER] deliveryman notification inactive', ['order_id' => $order->id, 'sms_status' => $dm_sms_status?->sms_status]);
                }
            } catch (\Exception $smsException) {
                \Illuminate\Support\Facades\Log::channel('daily')->error('[SMS ORDER] exception', ['order_id' => $order->id ?? null, 'message' => $smsException->getMessage(), 'line' => $smsException->getLine()]);
            }

            // ── Partner webhooks (order.created / order.status_changed) ──────
            // Fully independent try/catch so a webhook failure can never affect
            // SMS delivery or the order flow above.
            try {
                $isNewOrderEvent = (
                    ($order->order_status == 'pending' && in_array($order->payment_method, ['cash_on_delivery', 'offline_payment']))
                    || ($order->order_status == 'confirmed' && !in_array($order->payment_method, ['cash_on_delivery', 'offline_payment']))
                );
                $webhookEvent = $isNewOrderEvent ? 'order.created' : 'order.status_changed';

                $webhooks = \App\Models\PartnerWebhook::where('restaurant_id', $order->restaurant_id)
                    ->where('active', true)
                    ->get();

                foreach ($webhooks as $webhook) {
                    if (!$webhook->subscribesTo($webhookEvent)) {
                        continue;
                    }
                    \App\Jobs\DispatchPartnerWebhook::dispatch(
                        $webhook->id,
                        $webhookEvent,
                        $order->id,
                        (string) Str::uuid()
                ...
                \Illuminate\Support\Facades\Log::channel('daily')->error('[PARTNER WEBHOOK] dispatch exception', ['order_id' => $order->id ?? null, 'message' => $webhookException->getMessage(), 'line' => $webhookException->getLine()]);
            }
   ...
   public static function getNextOpeningTime($schedule) {
    ...
    public static function getActivePaymentGateways(){
        ...
        if($published_status == 1){
            ...
            $methods = DB::table('addon_settings')->where('is_active',1)
            ->whereIn('settings_type', ['payment_config'])->whereIn('key_name', 
            ['ssl_commerz','paypal','stripe','razor_pay','senang_pay','paytabs',
            'paystack','paymob_accept','paytm','flutterwave','liqpay','bkash','mercadopago','hyper_pay'])->get();
            $env = env('APP_ENV') == 'live' ? 'live' : 'test';
            $credentials = $env . '_values';
    ...
    public static function checkCurrency_bk($data , $type= null){
...
// public static function getPaymentGatewaySupportedCurrencies($gateway = null)
// {
//     $supported_currencies = [
//         'ssl_commerz' => ['BDT' => 'Bangladeshi Taka'],
//         'paypal' => ['USD' => 'US Dollar', 'EUR' => 'Euro', 'GBP' => 'British Pound', 'CAD' => 'Canadian Dollar', 'AUD' => 'Australian Dollar', 'JPY' => 'Japanese Yen', 'CNY' => 'Chinese Yuan', 'CHF' => 'Swiss Franc', 'SEK' => 'Swedish Krona', 'NZD' => 'New Zealand Dollar'],
//         'stripe' => ['USD' => 'US Dollar', 'EUR' => 'Euro', 'GBP' => 'British Pound', 'CAD' => 'Canadian Dollar', 'AUD' => 'Australian Dollar', 'SGD' => 'Singapore Dollar', 'HKD' => 'Hong Kong Dollar', 'CHF' => 'Swiss Franc', 'SEK' => 'Swedish Krona', 'DKK' => 'Danish Krone', 'NOK' => 'Norwegian Krone', 'PLN' => 'Polish Zloty', 'CZK' => 'Czech Koruna', 'HUF' => 'Hungarian Forint', 'RON' => 'Romanian Leu', 'BGN' => 'Bulgarian Lev', 'HRK' => 'Croatian Kuna', 'RUB' => 'Russian Ruble', 'TRY' => 'Turkish Lira', 'BRL' => 'Brazilian Real', 'MXN' => 'Mexican Peso', 'ARS' => 'Argentine Peso', 'CLP' => 'Chilean Peso', 'COP' => 'Colombian Peso', 'PEN' => 'Peruvian Sol', 'UYU' => 'Uruguayan Peso', 'INR' => 'Indian Rupee', 'IDR' => 'Indonesian Rupiah', 'MYR' => 'Malaysian Ringgit', 'PHP' => 'Philippine Peso', 'THB' => 'Thai Baht', 'VND' => 'Vietnamese Dong', 'KRW' => 'South Korean Won', 'TWD' => 'Taiwan Dollar', 'NTD' => 'New Taiwan Dollar', 'ZAR' => 'South African Rand', 'AED' => 'UAE Dirham', 'SAR' => 'Saudi Riyal', 'QAR' => 'Qatari Riyal', 'OMR' => 'Omani Rial', 'KWD' => 'Kuwaiti Dinar', 'BHD' => 'Bahraini Dinar', 'JOD' => 'Jordanian Dinar', 'ILS' => 'Israeli Shekel', 'EGP' => 'Egyptian Pound', 'PKR' => 'Pakistani Rupee', 'BDT' => 'Bangladeshi Taka'],
//         'razor_pay' => ['INR' => 'Indian Rupee'],
//         'senang_pay' => ['MYR' => 'Malaysian Ringgit'],
//         'paytabs' => ['AED' => 'UAE Dirham', 'SAR' => 'Saudi Riyal', 'QAR' => 'Qatari Riyal', 'OMR' => 'Omani Rial', 'KWD' => 'Kuwaiti Dinar', 'BHD' => 'Bahraini Dinar', 'JOD' => 'Jordanian Dinar', 'ILS' => 'Israeli Shekel', 'EGP' => 'Egyptian Pound', 'USD' => 'US Dollar', 'EUR' => 'Euro'],
//         'paystack' => ['NGN' => 'Nigerian Naira', 'GHS' => 'Ghanaian Cedi', 'KES' => 'Kenyan Shilling', 'ZAR' => 'South African Rand', 'USD' => 'US Dollar'],
//         'paymob_accept' => ['EGP' => 'Egyptian Pound'],
//         'paytm' => ['INR' => 'Indian Rupee'],
//         'flutterwave' => ['NGN' => 'Nigerian Naira', 'GHS' => 'Ghanaian Cedi', 'KES' => 'Kenyan Shilling', 'TZS' => 'Tanzanian Shilling', 'UGX' => 'Ugandan Shilling', 'ZAR' => 'South African Rand', 'XOF' => 'West African CFA Franc', 'XAF' => 'Central African CFA Franc', 'RWF' => 'Rwandan Franc', 'USD' => 'US Dollar', 'EUR' => 'Euro', 'GBP' => 'British Pound'],
//         'liqpay' => ['UAH' => 'Ukrainian Hryvnia', 'EUR' => 'Euro', 'USD' => 'US Dollar', 'RUB' => 'Russian Ruble'],
//         'bkash' => ['BDT' => 'Bangladeshi Taka'],
//         'mercadopago' => ['ARS' => 'Argentine Peso', 'BRL' => 'Brazilian Real', 'CLP' => 'Chilean Peso', 'COP' => 'Colombian Peso', 'MXN' => 'Mexican Peso', 'PEN' => 'Peruvian Sol', 'UYU' => 'Uruguayan Peso', 'USD' => 'US Dollar'],
//         'hyper_pay' => ['ILS' => 'Israeli Shekel', 'USD' => 'US Dollar', 'EUR' => 'Euro', 'GBP' => 'British Pound'],
//     ];

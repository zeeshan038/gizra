...
class OrderController extends Controller
{
    ...
    public function details(Request $request, $id)
    {
        ...
        if (isset($order)) {
            ...
            $notifiedDrivers = [];
            if ($order && $order->is_manual_dispatch == 1 && !$order->delivery_man_id && $order->restaurant) {
                $notifiedDrivers = \App\Models\DeliveryMan::where('zone_id', $order->restaurant->zone_id)->active()->get();
            }

            return view('admin-views.order.order-view', compact('order', 'deliveryMen', 'categories', 'products', 'category', 'keyword', 'editing', 'selected_delivery_man', 'notifiedDrivers'));
        } else {
    ...
    public function manual_list($status, Request $request)
    {
        $key = explode(' ', $request['search']);
        ...
        Order::where(['checked' => 0, 'is_manual_dispatch' => 1])->update(['checked' => 1]);

        $orders = Order::with(['customer', 'restaurant', 'delivery_man'])
            ->where('is_manual_dispatch', 1)
            ->when(isset($request->zone), function ($query) use ($request) {
                return $query->whereHas('restaurant', function ($q) use ($request) {
                    return $q->whereIn('zone_id', $request->zone);
                });
            ...
            ->when( !in_array($status,['all','scheduled','canceled','refund_requested','refunded','delivered','failed'])  , function ($query) {
                return $query->OrderScheduledIn(30);
            })
            ->when(isset($request->vendor), function ($query) use ($request) {
                return $query->whereHas('restaurant', function ($query) use ($request) {
        ...
        $is_manual = true;
        return view('admin-views.order.list', compact('orders', 'status', 'orderstatus', 'scheduled', 'vendor_ids', 'zone_ids', 'from_date', 'to_date', 'total', 'order_type', 'is_manual'));
    }
